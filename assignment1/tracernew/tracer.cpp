...
/*
void BeginInstruction_read(VOID *ip, UINT32 op_code, VOID *opstring,INS ins){
...
void BeginInstruction_write(VOID *ip, UINT32 op_code, VOID *opstring,INS ins){
//INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)EndInstruction, IARG_END);
	if(INS_IsMemoryRead(ins)||INS_IsMemoryWrite(ins)){
	//	INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)BeginInstruction, IARG_INST_PTR, IARG_UINT32, opcode, IARG_END);
	}
...
void countNonMemInstr(){
	nonmeminstCount++;
}

void recordReadInstr(INS ins){

if(INS_IsMemoryRead(ins)){
INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)countMemInstr, IARG_MEMORYREAD_EA, IARG_END);
//CYCLEVALS=CYCLEVALS+NONMEMDELAY;
}
}
void recordWriteInstr(INS ins){
if(INS_IsMemoryWrite(ins)){
INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)countMemInstr, IARG_MEMORYWRITE_EA, IARG_END);
}
}
void EndInstruction()
{
...
//	INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)BeginInstruction_mem, IARG_MEMORYREAD_EA, IARG_MEMORYREAD2_EA , opcode, IARG_END);
//	INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)EndInstruction, IARG_END);
if( INS_IsMemoryRead(ins)){
	recordReadInstr(ins);
	CYCLEVALS=CYCLEVALS+MEMDELAY;
}
else if(INS_IsMemoryWrite(ins)){
	recordWriteInstr(ins);
		CYCLEVALS=CYCLEVALS+MEMDELAY;
}
else{
INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)countNonMemInstr, IARG_INST_PTR, IARG_END);
	CYCLEVALS=CYCLEVALS+NONMEMDELAY;
}
//  	INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)BeginInstruction_mem, IARG_INST_PTR, IARG_UINT32, opcode, IARG_END);
//	INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)BeginInstruction_mem, IARG_INST_PTR, IARG_UINT32, opcode, IARG_END);
 //BeginInstruction_mem(ins);
